...
export default function ProductDetail() {
  ...
  const stockColor =
    stock > 50
      ...
      : stock > 10
        ? "bg-amber-400 text-amber-600 dark:text-amber-400"
        : "bg-red-500 text-red-600 dark:text-red-400";
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <p className="text-zinc-500 dark:text-zinc-400">Түр хүлээнэ үү...</p>
      </div>
    );
  }
  if (error) {
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-6 flex items-center gap-2">
              <span
                className={`h-2.5 w-2.5 rounded-full ${stockColor.split(" ")[0]}`}
              ></span>
              <span
                className={`text-sm font-medium ${stockColor.split(" ").slice(1).join(" ")}`}
              >
                {stock} left
              </span>
            </div>
